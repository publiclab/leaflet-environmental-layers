...
// well, we're trying to find nearest neighbors. Maybe if we don't find any matches, we can give up instead of searching ALL points?
    if (this.settings.data.hasOwnProperty('features')) {
      var coords = this.settings.data.features[0].geometry.coordinates;
      var copy = [coords[this.settings.latitudeKey], coords[this.settings.longitudeKey]];
...
console.log(latLng, closestFromEach)
  console.log(utils.pointInCircle(xy, e.layerPoint, instance.pointSize(), instance.settings.sensitivity))
  console.log(xy, e.layerPoint, instance.pointSize(), instance.settings.sensitivity) 
// this is returning false!
  if (utils.pointInCircle(xy, e.containerPoint, instance.pointSize() * instance.settings.sensitivity)) {
