fracTrackerMobileLayer = map => {
  let FracTracker_mobile  = L.esri.featureLayer({
  ...
  FracTracker_mobile.bindPopup(layer => {
  ...
  FracTracker_mobile.on('loading', e => {
    if(typeof map.spin === 'function'){
      map.spin(true) ;
    }
  });
  FracTracker_mobile.on('load', e => {
    if(typeof map.spin === 'function'){
      map.spin(false) ;
    }
  });
