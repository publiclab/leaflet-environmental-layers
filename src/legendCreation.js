$(window).one('load', function() {
  ...
  let legendPairs = {}; //key: layer name from overlayMaps; val: url for legend;
  
  //Add any legend using addLayerNameURLPair(layer_let, img_url);
  ...
  let legendControl = L.control.legendControl();
  legendControl.addTo(map);
  
  //Add for any layers that are already present on map load
  let activeLayers = leafletControl.getActiveOverlayNames();
  for(let i = 0; i < activeLayers.length; i++) {
    let layerName = activeLayers[i];
